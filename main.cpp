...
* Programa: Jogo da Memória 2.0
* Data de Criação: 28/10/2017
...
// Configuração de Resolução e Cores:
#define MAX_X		800 // Resolução Horizontal.
#define MAX_Y		600 // Resolução Vertical.
#define V_MAX_X     0
#define V_MAX_Y     0
#define COLOR_BITS	8

// Protótipos do Allegro:
int inicia(void);
void principal(void);
void finaliza(void);
...
int inicia (void)
{
	...
	install_keyboard(); // Função para instalar o teclado.
	install_mouse(); // Função para instalar o mouse.
	install_timer(); // Função para instalar o timer.
	set_color_depth(8); // Função de instalação do modo gráfico.
	...
	set_volume(150, 200); // Configuração de Volume.
	return (TRUE);
}
...
void principal (void)
{    
	int	x=165, y=165; // Posição da carta inicial/referência.
    int	i, ai; // Índice.
    ...
    int score1=0, score2=0; // Pontuação bruta.
	float sfinal1, sfinal2; // Pontuação líquida.
    int jogada1=0, jogada2=0; // Jogadas.
    int round=0; // Total de rodadas.   
    int esc=0; // Botão ESC.
	int	botEsq; // Botão esquerdo do mouse.
	int	botDir; // Botão direito do mouse.
    ...
    PALETTE	pal; // Declaração da paleta de cores.
	...
	play_midi(musica, TRUE); // Inicia a música de fundo.
	...
	// Loop do jogo com condição de fim:
	while (esc!=1 && (score1+score2)<10)
	{
		...
		// Texto de pontuação e informações de saída:
